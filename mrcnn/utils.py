...
def unmold_mask(mask, bbox, image_shape):
    ...
    """
    threshold = 0.5
    mask = resize(mask, (bbox[2] - bbox[0], bbox[3] - bbox[1]))
    mask = np.where(mask >= threshold, 1, 0).astype(np.bool)

    # Put the mask in the right location.
    full_mask = np.zeros(image_shape[:2], dtype=np.bool)
    full_mask[bbox[0]:bbox[2], bbox[1]:bbox[3]] = mask
    return full_mask


def unmold_mask_graph(mask, bbox, image_shape):
    """Converts a mask generated by the neural network to a format similar
    to its original shape.
    mask: [height, width] of type float. A small, typically 28x28 mask.
    bbox: [y1, x1, y2, x2]. The box to fit the mask in.
...
def norm_boxes(boxes, shape):
    ...
    """
    scale = [shape[0] - 1, shape[1] - 1, shape[0] - 1, shape[1] - 1]
    shift = [0, 0, 1, 1]
...
def delete_indexes(tensor, indexes):
    """
    Removes elements at `indexes` from `tensor`
    """
    mask = tf.ones(tf.shape(tensor), dtype=tf.bool)
    mask = tf.tensor_scatter_nd_update(mask, tf.reshape(indexes, [tf.shape(indexes)[0], 1]), False)
    return tf.boolean_mask(tensor, mask)

def unmold_detections_graph(detections, mrcnn_mask, original_image_shape,
                      image_shape, window):
    """Reformats the detections of one image from the format of the neural
    network output to a format suitable for use in the vector part.
